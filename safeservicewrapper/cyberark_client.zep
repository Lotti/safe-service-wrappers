...
class CyberarkClient
{
     ...
     */
    public static function fetchPassword(string host, string user) -> array
    {
        ...
        // --- Cache Check ---
        if cacheTtl > 0 {
            let cacheKey = md5(baseUrl . appId . safe . folder . host . user . certPath); // Include certPath in key
            ...
            if file_exists(cacheFilePath) {
                ...
                if typeof cachedData == "array" && isset cachedData["expires"] && isset cachedData["password"] && time() < cachedData["expires"] {
                    ...
                    // Cache expired or invalid, remove file
                    @unlink(cacheFilePath); // Use @ to suppress errors if file already gone
                }
            }
        }
        // --- End Cache Check ---
        ...
        let curlOptions = [
            ...
            \CURLOPT_USERAGENT: "zephir-cyberark-client-php/1.0",
        ...
        // Execute request
        let responseBody = curl_exec(ch);

        // Check for cURL errors during execution
        if responseBody === false {
            let errorMsg = curl_error(ch);
            curl_close(ch);
            throw new Exception("cURL request execution failed: " . errorMsg);
        }

        // Get HTTP response code
        let responseCode = curl_getinfo(ch, \CURLINFO_RESPONSE_CODE);
        ...
        // --- Store in Cache ---
        if cacheTtl > 0 && defined cacheFilePath { // Only cache if TTL > 0 and path was determined
             let cacheData = [
             ...
             if !is_dir(cachePath) {
                 // Attempt to create recursively, suppress errors if it fails (e.g., permissions)
                 @mkdir(cachePath, 0777, true);
             }
             // Write to cache file only if directory exists or was created
             if is_dir(cachePath) && is_writable(cachePath) {
                // Use @ to suppress errors during file writing (e.g., disk full)
